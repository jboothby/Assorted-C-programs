...
int main(int argc, char **argv){
	...
	int displayNum = 0;	// number of word pairs to display (if left at 0, displays all contents of hashtable)
				// this value is set by the command line arguments
	...
	int tablesize = 1024;				// initial size 1024 is picked to reduce unnecessary resizing of table for small input files
	...
	for( int i = fileStart; i < argc; i++){ 
		...
		char* lastWord = (char*)calloc( 100, sizeof( char* )); // allocate space for running buffer of last word seen
									// size 100 is chosen becuase there is an extremely low chance of
									// overflow since no 100char words exist in the english language
